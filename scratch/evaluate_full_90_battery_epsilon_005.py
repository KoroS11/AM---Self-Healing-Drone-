...
def run_300t_eval(actor, sc, alpha=(3.0, 1.5), barrier_margin=0.05):
...
def main():
    ...
    print("=" * 100)
    print("FULL 90-SCENARIO 300-TICK BATTERY AUDIT: EPSILON = 0.05m vs EPSILON = 0.00m")
    print("=" * 100)
    ...
    res_50 = [run_300t_eval(actor_bp, sc, barrier_margin=0.05) for sc in all_50]
    ...
    print("\n[PART B: ALL STRESS CATEGORIES (300 TICKS, EPSILON=0.05m)]")
    stress_records = []
    
    for cat_name in cat_order:
        sc_list = all_scenarios[cat_name]
        cat_res = [run_300t_eval(actor_bp, sc, barrier_margin=0.05) for sc in sc_list]
        for r in cat_res:
            r["category"] = cat_name
    ...
    df_all.to_csv("scratch/full_90_battery_eps005_results.csv", index=False)
    print("\nSaved full results to scratch/full_90_battery_eps005_results.csv")
