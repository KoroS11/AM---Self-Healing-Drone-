"""
TEST LEXICOGRAPHIC CBF-QP POLICY
=================================
Evaluates Lexicographic CBF-QP filter on B-Prime across Category 11 and Benchmark.
"""
...
class LexicographicCBFGNNPolicy(GNNPolicy):
    ...
    1. Disconnected Phase (A <-> B not connected):
       Executes raw GNN policy forces with zero CBF restriction, ensuring 100% gap-closing mobilization.
    2. Connected Phase (A <-> B connected):
       Applies Second-Order CBF-QP safety filter to all live edges using exact min(Rc_i, Rc_j) radii,
       damping velocity jitter and preventing link breakage or limit-cycle chattering.
    """
    def __init__(
        self,
    ...
    def compute_control_forces(
        ...
        # Lexicographic switching:
        if not self._is_network_connected():
            # Disconnected phase: unrestricted gap closure
            final_acc = raw_world_acc
        else:
            # Connected phase: apply CBF-QP safety filter on all live edges
...
def run_eval():
    ...
    print("=" * 90)
    print("  EVALUATING LEXICOGRAPHIC CBF-QP FILTER ON B-PRIME (CATEGORY 11)")
    print("=" * 90)
    ...
    for sc in CAT11_SCENARIOS:
        ...
        policy = LexicographicCBFGNNPolicy(sim, actor_gnn=actor_bp, alpha1=2.0, alpha2=1.0, barrier_margin=0.0, device="cpu")
        ...
        print(f"  First Reconnect T:   {first_conn} ({first_conn - 10 if first_conn else None}t relative)")
        print(f"  Snap @ t=150:        {snap_150}")
        print(f"  Snap @ t=300:        {snap_300}")
        print(f"  Sustained K=20 @ 150:{k20_150}")
        print(f"  Sustained K=20 @ 300:{k20_300}")
        print(f"  Post-Reconn Drops:   {drops_after}")
        print(f"  Min Slack @ t=150:   {df.loc[149, 'min_slack']:.2f}m")
        print(f"  Min Slack @ t=300:   {df.loc[299, 'min_slack']:.2f}m")
        print(f"  Final Sum-Rate:      {df.loc[299, 'sum_rate']:.2f} Mbps")

        # Save trace
        df.to_csv(f"scratch/cat11_cbf_scenario_{sc['id']}_trace.csv", index=False)
